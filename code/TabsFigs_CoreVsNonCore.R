...
# Core 
  read.csv("data/surveyWide_core.csv") -> surv_core  
  read.csv("data/bySite_core.csv") %>%
    transmute(year = Year, site = Site_ID, pots = Pot_Count, all_cnt = Total_Spot_Count, all_lb = Total_Spot_Wt_KG * 2.20462, propLrg = Proportion_Large, 
           lrg_cnt = Est_Count_LG, lrg_lb = Est_Wt_Large * 2.20462, cpue_all_lbs = CPUE_All_LB, cpue_all_cnt = CPUE_All_Count, cpue_lrg_cnt=CPUE_Large_Count) ->site_core
# NonCore 
  read.csv("data/surveyWide_from17SS.csv") -> surv_all  
  read.csv("data/bySite_from17SS.csv") %>%
    transmute(year = Year, site = Site_ID, pots = Pot_Count, all_cnt = Total_Spot_Count, all_lb = Total_Spot_Wt_KG * 2.20462, propLrg = Proportion_Large, 
              lrg_cnt = Est_Count_LG, lrg_lb = Est_Wt_Large * 2.20462, cpue_all_lbs = CPUE_All_LB, cpue_all_cnt = CPUE_All_Count, cpue_lrg_cnt=CPUE_Large_Count) ->site_all
...
#Aggregate SURVEY 
  # join statArea and ShrimpArea to CPUE 
    site_core %>% select(year,site,pots,all_lb,lrg_lb) %>%  
    left_join (
      siteStatLUT %>% select(-Comments), by = c("site" = "SiteNum")) -> cpueBySite_core
...
#ALL 
    ...
    site_all %>% select(year,site,pots,all_lb,lrg_lb) %>%  
      left_join (
        siteStatLUT %>% select(-Comments), by = c("site" = "SiteNum")) -> cpueBySite_all
...
#CORE 
  read.csv('./P04_2017BOF/output/var_byYear_xz_w17_core.csv') -> var_byYear_core 
  var_byYear_core %>%  transmute (year,
                            all_core = 2.20462 * se_all_kg,
                            lrg_core = 2.20462 * se_lrg_kg) -> se_byYear 
  se_byYear %>% gather(class, se, c(all_core, lrg_core)) -> se_byYear_l_core

  surv_core %>% select (year = Year, all_core = CPUE_All_LB, lrg_core = CPUE_Large_LB) %>%
    gather(class, cpue_lb, c(all_core, lrg_core)) -> surv_l_core
  
  surv_l_core %>% group_by(class) %>% mutate (avg = mean(cpue_lb, na.rm = TRUE)) -> surv_l_core # calc longterm avgs

#All 
  read.csv('./P04_2017BOF/output/var_byYear_xz_w17.csv') -> var_byYear_all 
  var_byYear_all %>%  transmute (year,
                                  all_all = 2.20462 * se_all_kg,
                                  lrg_all = 2.20462 * se_lrg_kg) -> se_byYear 
  se_byYear %>% gather(class, se, c(all_all, lrg_all)) -> se_byYear_l_all

  surv_all %>% select (year = Year, all_all = CPUE_All_LB, lrg_all = CPUE_Large_LB) %>%
    gather(class, cpue_lb, c(all_all, lrg_all)) -> surv_l_all
  
  surv_l_all %>% group_by(class) %>% mutate (avg = mean(cpue_lb, na.rm = TRUE)) -> surv_l_all # calc longterm avgs
  
surv_l_core %>% left_join(se_byYear_l_core) %>%
  ggplot(aes(x = year, y = cpue_lb, group = class, colour = class) )+
          ...
          geom_hline(yintercept = unique(surv_l_core$avg), colour = grey(c(.1,.5)), lty = 'dashed')
#ggsave("./figs/surveyWideCPUE_lbs_wVar_w17_core.png", dpi=300, height=4.0, width=6.5, units="in")

surv_l_all %>% left_join(se_byYear_l_all) %>%
  ggplot(aes(x = year, y = cpue_lb, group = class, colour = class) )+
  scale_color_grey(start=.1, end=0.5,  name = '', labels = c("All Sizes", "Larges (>32mm)")) +
  theme(legend.position = c(.2,.8)) +
  scale_x_continuous(breaks = seq(1990,2016,2))  +
  ...
  geom_line () +
  geom_errorbar(aes(ymin=cpue_lb-se, ymax=cpue_lb+se, width = 0),position = position_dodge(width = 0.00)) + 
  geom_hline(yintercept = unique(surv_l_all$avg), colour = grey(c(.1,.5)), lty = 'dashed')    

# Combine core and all on one plot 
rbind(surv_l_all,surv_l_core) -> surv_l_comb 
rbind(se_byYear_l_all,se_byYear_l_core) -> se_byYear_l_comb 
